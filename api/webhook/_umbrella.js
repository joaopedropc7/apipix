...
module.exports = async (req, res, tokenKey = 'utmify_token') => {
  ...
  if (p.type !== 'transaction') {
    await addLog('info', 'webhook', `Evento Umbrella ignorado (tipo ${p.type})`, p);
  ...
  if (!newStatus) {
    await addLog('warn', 'webhook', `Status Umbrella não mapeado: ${p.data?.status}`, p);
    return res.status(200).json({ ok: true, warning: `Status não mapeado: ${p.data?.status}` });
  }

  if (!txId) {
    await addLog('warn', 'webhook', 'Webhook Umbrella sem objectId/data.id', p);
    return res.status(200).json({ ok: true });
  }
  ...
  if (!transaction) {
    await addLog('warn', 'webhook', `Transação Umbrella não encontrada: ${txId}`, p);
  ...
  await addLog('info', 'webhook',
    `Pagamento Umbrella ${newStatus === 'paid' ? 'CONFIRMADO ✅' : newStatus.toUpperCase()}: pedido #${transaction.request_number}`,
